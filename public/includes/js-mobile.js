	...
	$('#nav_load').click(function(){
		...
		socket.emit('tt_load',function(jsonO){
			...
			$('#modal_tt').children('div').each(function(){
				$(this).append('<div class = "col-xs-2"><button type="button" class="delete_tt_button btn btn-danger" id="id_button_tt_delete_'+escapeHTML($(this).children('div:first-child').html())+'">Delete</button></div>');
			});
...
function append_tutor_entry(json){	
	...
	'<div class = "col-xs-7 col-sm-7 col-md-3">'+escapeHTML(json.name)+'</div>'+
	'<div class = "hidden-xs hiddem-sm visible-md-3 visible-lg-3">'+escapeHTML(json.mobileno)+'</div>'+
	'<div class = "hidden-xs hidden-sm visible-md-4 visible-lg-4">'+escapeHTML(json.email)+'</div>'+
...
function populate_tt_modal(jsonO){
	for (i=0;i<Object.keys(jsonO).length;i++){
		...
		.append('<div class = "col-xs-7 col-sm-7 col-md-3">'+escapeHTML(jsonO[i][0])+'</div>')
		.append('<div class = "hidden-xs hidden-sm visible-md-7 visible-lg-7">'+escapeHTML(jsonO[i][1].split('T')[0])+'</div>')
	}
}
